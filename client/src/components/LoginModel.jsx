  ...
  return (
    <>
      <AnimatePresence>
        {open && (
          <motion.div
            className="fixed top-0 inset-0 z-100 flex items-center justify-center bg-black/80 backdrop-blur-xl px-4"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          >
            ...
            >
              <div className="relative rounded-3xl bg-[#0b0b0b] border border-white/10 shadow[0_30px_120px_rgba(0,0,0,0,0.8)] overflow-hidden">
                ...
                />
                <button className="absolute top-5 right-5 z-20 text-zinc-400 hover:text-white transition text-lg border px-2 rounded-full cursor-pointer"
                onClick={onClose}>
                  X
                </button>
